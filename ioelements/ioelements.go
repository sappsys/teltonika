...
func (r *Decoder) selectDefinition(modelName string, id uint16, wireSize int) (*IOElementDefinition, error) {
	...
	for i := range r.definitions {
		...
		// For unknown model ('*') with a known wire size, prefer the most widely
		// supported size match so single-device quirks (e.g. FMB930 External Voltage
		// ×0.01) do not win over the common fleet definition.
		if wireSize > 0 && definitionMatchesWireSize(def, wireSize) {
